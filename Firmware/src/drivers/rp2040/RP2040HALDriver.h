...
class Rp2040HALDriver : public ILvglPlatformDriver {
  ...
  private:
    ...
    lv_indev_t* m_lvglInputDevice = nullptr; // Add member to hold the input device (even if null for now)
    ...
    static void FlushCallbackStatic(lv_display_t* pDisp, const lv_area_t* pArea, uint8_t* pixmap);
