...
#include "UI/LvglUI.h"
// #include "drivers/ili9341/ILI9341Display.h" // Driver included via RP2040HALDriver
 ...
 * this function can return a fixed value. Returning 0 implies 0% idle (100% CPU usage).
 * @return uint32_t Idle percentage (0-100).
 */
uint32_t custom_get_idle_percent(void) {
    return 0; // Or any other fixed value, e.g., 100 if you want to imply 0% CPU usage.
              // For FPS monitoring, the exact value here is less critical.
}
...
// const int LVGL_TICK_PERIOD_MS = 17; // Adjusted to match LV_DEF_REFR_PERIOD, or try 15ms
...
int main()
{
    ...
    // This ensures that lv_task_handler() or lv_refr_now() sees a dirty screen for the first frame.
    if (platformDriver && platformDriver->GetLvDisplay()) {
        ...
        if(active_screen_on_disp) {
            ...
            printf("Main.cpp: Active screen on display explicitly invalidated for initial draw.\n");
        }
    }
    ...
    // fps_last_update_time = std::chrono::steady_clock::now(); // Initialize FPS timer
    ...
    while (true)
    {
        ...
        if (g_request_lvgl_refresh) {
            ...
            if (platformDriver && platformDriver->GetLvDisplay()) {
                // printf("MainLoop: Calling lv_display_refr_timer() due to TE pulse.\n"); // Verbose
                // lv_refr_now(platformDriver->GetLvDisplay()); // lv_refr_now is immediate, can be heavy in main loop
                lv_display_refr_timer(nullptr); // Schedules a refresh for the next lv_task_handler call
            }
        }

        // Periodically print lv_task_handler duration, e.g., once per second along with FPS
        // This will be grouped with the FPS update logic below.

        // --- FPS Calculation ---
        // fps_frame_count++;
        // auto current_time = std::chrono::steady_clock::now();
        // auto elapsed_ms = std::chrono::duration_cast<std::chrono::milliseconds>(current_time - fps_last_update_time).count();
        ...
        //         snprintf(fps_buf, sizeof(fps_buf), "FPS: %.1f\nHandler: %lu us", fps_val, lv_handler_duration_us);
